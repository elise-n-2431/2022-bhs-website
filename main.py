...
def home():
    ...
    thing = connectsql("db/Divisionalpoints.db", "SELECT north, south, west\
                       FROM Points")
    ...
    for division in divisions:
        third = division
    ''' define division values, name and colour eg.num1=largest number,
    name3=name of div with least points etc. to display'''
    num1 = first[1]
    num2 = second[1]
    num3 = third[1]
    name1 = first[0]
...
def clubs():
    with sqlite3.connect("db/Clubs.db") as connection:
        ...
        if request.method == 'POST':
            ...
            # if the categories and days have sumbitted 0, select everything
            if categories == '0'and dayvar == '0':
                cursor = connection.cursor()
                cursor.execute('SELECT Club.title, Club.room, Club."desc",\
                               Club.contact, Club.teachcode, Club.category,\
                               club.restrictions FROM Club ORDER BY\
                               Club.title')
                ...
                cursor.execute('SELECT Club.title, Club.room, Club."desc",\
                               Club.contact, Club.teachcode, Club.category,\
                               club.restrictions, club.id FROM Club\
                               ORDER BY Club.title')
                fetch = cursor.fetchall()
                # select all clubs from database
                for club in fetch:
                    if dayvar == '0':
                        '''if they selected all days + a category, add all clubs
                        which match the category to a list'''
                        cat = int(club[5])
                        ...
                        query = 'SELECT DaysClubs.daysid FROM DaysClubs\
                        JOIN Club ON Club.id=DaysClubs.clubid WHERE Club.id=?'
                        cursor.execute(query, (club[7], ))
                        days = cursor.fetchall()
                        for day in days:
                            ''' for each day which the club is on, check if it
                            matches the day selected by the user'''
                            dayvar = int(dayvar)
                            day = int(day[0])
                            if day == dayvar:
                                ''' if the club is on on the selected day,
                                check if the club matches the user's
                                selected category'''
                                cat = int(club[5])
                                categories = int(categories)
                                if cat == categories:
                                ...
                                if categories == 0:
                                    clubs.append(club)
                                ''' add to a list if the user selected all
                                categories or the category selected matches
                                the category of the club'''
        else:
            # select everything from categories and days from clubs database
            cursor = connection.cursor()
            cursor.execute('SELECT Club.title, Club.room, Club."desc",\
                           Club.contact, Club.teachcode, Club.category,\
                           club.restrictions FROM Club')
            clubs = cursor.fetchall()
                ...
                if check(north) == True:
                    ...
                    if check(south) == True:
                        ...
                        if check(west) == True:
                            ...
                            with sqlite3.connect("db/Divisionalpoints.db")\
                                    as connection:
                                insert = connection.cursor()
                                query = ('INSERT INTO Points(north, south, \
                                       west, event, date) VALUES (?, ?, ?, ?, ?)')
                                insert.execute(query, (north, south, west, \
                                               event, datet))
